...
#
# - https://blocksds.skylyrac.net/grit/
# - https://codeberg.org/blocksds/libnds/src/branch/master/include/nds/arm9/grf.h
# - https://codeberg.org/blocksds/libnds/src/branch/master/source/arm9/grf.c
...
class GenericBinary():
    ...
    def run_command_line_arguments(self, args=None, ninja_file_path='build.ninja',
        ...
        if build:
            ...
            my_env["BLOCKSDSEXT"] = BLOCKSDSEXT
            my_env["WONDERFUL_TOOLCHAIN"] = WONDERFUL_TOOLCHAIN
    ...
    def _gen_rules_tools(self):
        ...
        self.print(
            ...
            f'OBJ2DL_NEA      = python3 {blocksdsext}/nitro-engine-advanced/tools/obj2dl/obj2dl.py\n'
            f'MD5_TO_DSMA_NEA = python3 {blocksdsext}/nitro-engine-advanced/tools/md5_to_dsma/md5_to_dsma.py\n'
            f'IMG2DS          = python3 {blocksdsext}/nitro-engine-advanced/tools/img2ds/img2ds.py\n'
        )
        ...
        self.print(
            ...
            '\n'
            'rule obj2dl_nea\n'
            '  command = ${OBJ2DL_NEA} --input ${in_path_obj} --output $out ${args}\n'
            '\n'
            'rule md5_to_dsma_nea\n'
            '  command = ${MD5_TO_DSMA_NEA} ${args}\n'
            '\n'
            'rule img2ds\n'
            '  command = ${IMG2DS} --input ${in_path_png} --name ${name} --output ${out_path_dir} --format ${format}\n'
            '\n'
            'rule ptexconv\n'
...
class GenericArmBinary(GenericCpuBinary):
    ...
    def _gen_rules_source_arm(self, in_files, in_dirs, out_dir,
        ...
        - 'in_files': List of individual source code files.
        ...
        in_out_files = []
        for in_dir in in_dirs:
            dir_files = gen_input_file_list(in_dir, ('.c', '.cpp', '.s'))
            in_out_files.extend(gen_out_file_list(dir_files, '', out_dir + '/', '', '.o'))
    ...
    def generate_elf(self):
        ...
        self._gen_rules_source_arm(in_files=self.sourcefiles,
                                   in_dirs=self.sourcedirs, out_dir=self.out_dir,
                                   asflags=asflags, cflags=cflags, cxxflags=cxxflags,
                                   assets_c_files=self.assets_c,
                                   assets_h_flag=self.flag_assets_name)
...
class Arm9Binary(GenericArmBinary):
    ...
    def __init__(self, *, sourcefiles=[], sourcedirs=[], defines=[], includedirs=[],
                 libs=['nds9', 'mm9'],
        ...
        - 'sourcefiles': List of source code files.
        ...
        self.defines = ['__NDS__', '__BLOCKSDS__', 'ARM9'] + defines
        ...
        self.arch = '-mthumb -mcpu=arm946e-s+nofp'
    ...
    def add_nitro_engine_advanced_obj(self, in_dirs, out_dir='models'):
        '''
        Nitro Engine Advanced: same as add_nitro_engine_obj but uses the
        nitro-engine-advanced obj2dl. Input directories are scanned for '.obj'
        files; each must have a matching '.json' sidecar with at least a
        "texture" field.
        '''
        full_out_dir = os.path.join(self.out_assets_path, out_dir)
        ...
        for in_out_file in in_out_files:
            ...
            self.print(
                f'build {out_path_dl} : obj2dl_nea {in_path_obj} {in_path_json} || {out_path_dir}\n'
                f'  in_path_obj = {in_path_obj}\n'
                f'  args = {args}\n'
                '\n'
            )
    ...
    def add_nitro_engine_advanced_md5(self, in_dirs, out_dir='models'):
        '''
        Nitro Engine Advanced: same as add_nitro_engine_md5 but uses the
        nitro-engine-advanced md5_to_dsma. The json sidecar additionally
        accepts:
        ...
        for in_out_file in md5mesh_in_out_files:
            ...
            if json_data.get('blender-fix'):
                args += ' --blender-fix'
            if json_data.get('no-strip'):
                args += ' --no-strip'
            if json_data.get('multi-material'):
                args += ' --multi-material'
            if json_data.get('old-md5'):
                args += ' --old-md5'
            ...
            self.print(
                f'build {out_path_dsm} : md5_to_dsma_nea {in_path_md5mesh} {in_path_json} || {out_path_dir}\n'
                f'  args = {args}\n'
                 '\n'
            )
            ...
            if 'animations' in json_data:
                ...
                for animation in json_data['animations']:
                    ...
                    self.print(
                        f'build {out_path_dsa} : md5_to_dsma_nea {in_path_md5anim} {in_path_json} || {out_path_dir}\n'
                        f'  args = {args}\n'
                         '\n'
                    )
    ...
    def add_img2ds(self, in_dirs, out_dir='textures'):
        '''
        Nitro Engine Advanced: scans the given directories for '.png' files.
        Each png must have a matching '.json' sidecar specifying the output
        format, one of: A1RGB5, PAL256, PAL16, PAL4, A3PAL32, A5PAL8, DEPTHBMP.

            { "format": "PAL256" }

        Produces <name>_tex.bin and, for palettized formats, <name>_pal.bin.
        '''
        PALETTIZED = {'PAL256', 'PAL16', 'PAL4', 'A3PAL32', 'A5PAL8'}
        ...
        for in_out_file in in_out_files:
            ...
            in_path_json = replace_ext(in_path_png, '.png', '.json')

            json_data = load_json(in_path_json)
            assert 'format' in json_data, 'Texture format must be provided'
            fmt = json_data['format']

            name = remove_ext(get_file_name(in_path_png))

            out_path_tex = os.path.join(out_path_dir, f'{name}_tex.bin')
            outs = out_path_tex
            if fmt in PALETTIZED:
                out_path_pal = os.path.join(out_path_dir, f'{name}_pal.bin')
                outs += ' ' + out_path_pal

            self.print(
                f'build {outs} : img2ds {in_path_png} {in_path_json} || {out_path_dir}\n'
                f'  in_path_png = {in_path_png}\n'
                f'  name = {name}\n'
                f'  out_path_dir = {out_path_dir}\n'
                f'  format = {fmt}\n'
                '\n'
            )
...
class Arm9DynamicLibrary(GenericArmBinary):
    ...
    def __init__(self, *, name, main_binary=None,
                 sourcefiles=[], sourcedirs=[], defines=[],
...
class Arm7Binary(GenericArmBinary):
    ...
    def __init__(self, *, sourcefiles=[], sourcedirs=[], defines=[], includedirs=[],
        ...
        self.defines = ['__NDS__', '__BLOCKSDS__', 'ARM7'] + defines
        ...
        self.arch = '-mthumb -mcpu=arm7tdmi'
...
class TeakBinary(GenericCpuBinary):
    ...
    def __init__(self, *, name, sourcefiles=[], sourcedirs=[],
                 defines=[], includedirs=[],
                 libs=['teak'],
        ...
        self.defines = ['__NDS__', '__BLOCKSDS__', 'TEAK'] + defines
    ...
    def _gen_rules_source_teak(self, in_files, in_dirs, out_dir,
                               asflags, cflags, cxxflags,
                               assets_c_files, assets_h_flag):
        '''
        Generates rules to build all provided source files, and adds additional
    ...
    def generate_tlf(self):
        ...
        self._gen_rules_source_teak(in_files=self.sourcefiles,
                                    in_dirs=self.sourcedirs, out_dir=self.out_dir,
                                    asflags=asflags, cflags=cflags, cxxflags=cxxflags,
                                    assets_c_files=self.assets_c,
                                    assets_h_flag=self.flag_assets_name)
...
class GenericFilesystem(GenericBinary):
    ...
    def add_ptexconv(self, in_dirs, out_dir='ptexconv'):
        ...
        them with the same name but with the extension '.ptxc'. The, it will create
        rules to convert them to an NDS format with ptexconv using the arguments supplied
        in the .ptxc file and add them to the filesystem as '.bin' files (or as
        a single '.grf' file if '-og' is found in the arguments).
        '''
        full_out_dir = os.path.join(self.out_assets_path, out_dir)
        ...
        for in_out_file in in_out_files:
            ...
            # Parse all lines in the file and concatenate them
            in_args = ""
            with open(f'{os.path.splitext(in_out_file.in_path)[0]}.ptxc', 'r') as arg_file:
                line = arg_file.readline().strip('\n')
                if not line.startswith('#'):
                    in_args += " " + line;
            assert len(in_args) > 0

            if '-og' in in_args:
                # Write rules for a GRF output file

                out_path_grf = ptexconv_out_path + '.grf'

                build_args = f'{out_path_grf}'

                self.target_files.append(out_path_grf)
            else:
                # Write rules for a BIN output file
    ...
    def add_nitro_engine_advanced_obj(self, in_dirs, out_dir='models'):
        '''
        Nitro Engine Advanced: filesystem variant of add_nitro_engine_advanced_obj.
        Uses the nitro-engine-advanced obj2dl. Scans the given directories for
        '.obj' files, each accompanied by a matching '.json'.
        '''
        full_out_dir = os.path.join(self.out_assets_path, out_dir)
    ...
    def add_nitro_engine_advanced_md5(self, in_dirs, out_dir='models'):
        '''
        Nitro Engine Advanced: filesystem variant of add_nitro_engine_advanced_md5.
        Uses the nitro-engine-advanced md5_to_dsma. The json sidecar additionally
        accepts "no-strip", "multi-material", and "old-md5" boolean keys.
        '''
        full_out_dir = os.path.join(self.out_assets_path, out_dir)
    ...
    def add_img2ds(self, in_dirs, out_dir='textures'):
        '''
        Nitro Engine Advanced: filesystem variant. Scans for '.png' files,
        each paired with a '.json' specifying "format" (A1RGB5, PAL256, PAL16,
        PAL4, A3PAL32, A5PAL8, DEPTHBMP). Produces <name>_tex.bin and, for
        palettized formats, <name>_pal.bin.
        '''
        PALETTIZED = {'PAL256', 'PAL16', 'PAL4', 'A3PAL32', 'A5PAL8'}
...
class NdsRom(GenericBinary):
    ...
    def __init__(self, *, nitrofsdirs=[],
        ...
        - 'game_icon': Icon to be used in the ROM header (PNG, BMP or GIF).
